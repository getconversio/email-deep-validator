...
class EmailVerifier {
  constructor(options = { timeout: 10000, verifyMxRecords: true, verifySmtpConnection: true }) {
    this.options = options;
  }

  verify(address) {
    const domain = EmailVerifier.extractDomain(address);

    return Promise.resolve()
      .then(() => {
        if (!this.options.verifyMxRecords) return;

        return EmailVerifier.resolveMxRecords(address)
          .then(mxRecords => {
  ...
  static extractDomain(address) {
    if (!EmailVerifier.isEmail(address)) {
